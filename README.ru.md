...
Один файл уходит в один контейнер под паролем и возвращается обратно - из командной строки, из меню
Проводника или со страниц группы **Защита** в окне. Настоящее имя оригинала, его реальный размер и
отметки времени запечатаны внутри; сам контейнер выдаёт только собственный размер, своё видимое имя и
время создания (`rename` делает безымянный блок данных).
...
- **Надёжная идентификация на основе MD5**
...
- **Стандартная альтернativa** - удаление файл за файлом с прогрессом для защищённых папок
...
> **Тестовые файлы**: Создаёт файлы `FILL_*.tmp` и `speedtest_*.txt`. Используйте команду `clean` для их автоматического удаления.
...
├── capacitytest/             # Модуль тестирования ёмкости
