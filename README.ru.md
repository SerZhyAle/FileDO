...
*Инструменты для приятно параноидальных - потому что некоторые диски врут о своём размере, а вы заслуживаете доказательств.*
...
Ставит инструменты командной строки (`filedo`, `filedo_check`, `filedo_fill`, `filedo_test`) и графический конструктор команд `filedo_win`, и добавляет их в `PATH`.

#### Вариант 2 - Microsoft Store (MSIX)

Один пакет, два входа: плитка **FileDO** - графический интерфейс, который соберёт и запустит любую команду, и команда `filedo` в `PATH` для любого терминала.
...
2. **GUI**: в архиве есть `filedo_win.exe` - запускайте рядом с `filedo.exe` для визуального конструктора команд.
...
**FileDO GUI** (`filedo_win.exe`) - конструктор команд на VB.NET (Windows Forms), который соберёт и запустит **любую** команду FileDO:
...
- ✅ **Все операции** в одном списке: info, speed, test, fill, clean, cd (дубликаты), всё семейство copy (copy / fastcopy / synccopy / balanced / maxcopy / smartcopy / safecopy), wipe, compare, check, probe, recover, from, hist
...
- ✅ **Живой предпросмотр** в редактируемом поле - правьте вручную всё, что не покрыл конструктор
...
- ✅ **Окно «О программе»** (кнопка **О программе** справа сверху): сборка, автор и ссылки на сайт, исходный код, трекер проблем, страницу приватности и другие программы автора
- ✅ **Отправить логи автору** - единственная кнопка в этом окне собирает найденные на машине логи FileDO в один zip, открывает папку с выделенным файлом, копирует путь в буфер обмена и открывает вашу почтовую программу с уже заполненным адресом и темой. Zip вы сначала видите сами, и ничего не уходит, пока вы сами не отправите письмо
...
**v2605152056**
- **Установщик**: MSI-установщик; значок приложения встроен во все EXE и в запись в списке программ
